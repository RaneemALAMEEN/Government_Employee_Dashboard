...
class _TextFieldWidgetState extends State<_TextFieldWidget> {
  ...
  TextInputType _resolveKeyboardType(String? inputType, String label) {
    final type = inputType?.toLowerCase().trim();
    if (type == 'int' ||
        type == 'number' ||
        type == 'integer' ||
        type == 'digits') {
  ...
  List<TextInputFormatter> _resolveInputFormatters(
    ...
    if (isInt || isPhone) {
      formatters.add(FilteringTextInputFormatter.allow(RegExp(r'[0-9٠-٩]')));
  ...
  @override
  Widget build(BuildContext context) {
    final inputType = widget.widgetEntity.data['input_type']?.toString();
    final label = widget.widgetEntity.data['label']?.toString() ?? '';
    final keyboardType = _resolveKeyboardType(inputType, label);
    final inputFormatters = _resolveInputFormatters(inputType, label);
