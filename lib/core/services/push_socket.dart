...
///
/// نستخدم [IOWebSocketChannel] مع `HttpClient` يتجاوز التحقق من الشهادة (مطابق
/// لـ DioClient/AuthInterceptor في هذا المشروع)، لأن التطبيق يتجاوز الشهادات
/// عالميًّا؛ فلو استخدمنا الاتصال المشترك قد تفشل مصافحة الـ wss على بعض البيئات.
///
...
class PushSocket {
  ...
  Future<void> _connect() async {
    ...
    // المشترك). نبني الـ Uri عبر replace كي يُرمَّز التوكن بشكل صحيح.
    ...
    try {
      // HttpClient يتجاوز التحقق من الشهادة (كبقية المشروع) كي لا تفشل مصافحة
      // الـ wss على بيئات الشهادات الذاتية/الوسيطة.
  ...
  ///     • فشل التجديد → ندخل وضع "انتظار الجلسة" (إعادة محاولة بطيئة 60ث) بلا
  ///       تنقّل؛ AuthInterceptor يتولّى مسح الجلسة والتوجيه لـ /login عند أول
  ///       طلب HTTP لاحق.
